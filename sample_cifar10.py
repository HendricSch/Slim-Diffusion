...
'''
Dieses Script kann genutzt werden, um ein Bild aus dem CIFAR-10 Datensatz zu sampeln. Dazu werden die Gewichte eines
vorher trainierten Modells geladen.
'''
...
def main():
    ...
    data = datasets.CIFAR10(img_size=IMAGE_SIZE)
    model = models.UNet(data.channels, data.img_size, T, [16, 32, 64, 128]).to(DEVICE)
    model.load_state_dict(torch.load("weights/cifar-10.pt"))
